...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── 3. NIGHT SKY CANVAS (stars, moon, clouds) ────────────
  (function initNightSky() {
    ...
    const stars = Array.from({ length: STAR_COUNT }, () => ({
      x: Math.random(),
      y: Math.random() * 0.70, // only in upper 70% (sky region)
    ...
    // ── Moon ──
    const moon = {
      relX: 0.82,  // 82% from left
      relY: 0.12,  // 12% from top
      r: 0,        // set on draw based on canvas size
    };
    ...
    const clouds = Array.from({ length: CLOUD_COUNT }, (_, i) => ({
      x: Math.random(),
      y: 0.05 + (i * 0.05),
      speed: 0.000018 + Math.random() * 0.000012, // very slow drift
      opacity: 0.06 + Math.random() * 0.06,       // very low opacity
      scale: 0.8 + Math.random() * 0.6,
      phase: Math.random() * Math.PI * 2,
    }));

    function drawMoon(ctx, w, h) {
      const cx = moon.relX * w;
      const cy = moon.relY * h;
      const r = Math.min(w, h) * 0.072;

      // Soft outer halo
      const halo = ctx.createRadialGradient(cx, cy, r * 0.8, cx, cy, r * 2.2);
      halo.addColorStop(0, 'rgba(240, 215, 150, 0.12)');
      halo.addColorStop(0.5, 'rgba(240, 215, 150, 0.05)');
      halo.addColorStop(1, 'rgba(240, 215, 150, 0)');
      ctx.fillStyle = halo;
      ctx.beginPath();
      ...
      // Moon glow
      const moonGlow = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
      moonGlow.addColorStop(0, 'rgba(255, 244, 200, 0.95)');
      moonGlow.addColorStop(0.5, 'rgba(240, 220, 160, 0.88)');
      moonGlow.addColorStop(1, 'rgba(200, 175, 110, 0.75)');

      // Crescent: draw full circle then cut with dark arc
      ctx.save();
      ctx.beginPath();
      ...
      ctx.beginPath();
      ctx.arc(cx - r * 0.28, cy - r * 0.08, r * 0.82, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalCompositeOperation = 'source-over';
      ctx.restore();
    }

    function drawCloud(ctx, cloud, w, h) {
      const cx = (cloud.x % 1.3) * w;
      const cy = cloud.y * h;
      const s = cloud.scale * Math.min(w, h) * 0.12;
      ctx.save();
      ctx.globalAlpha = cloud.opacity;
      ctx.fillStyle = 'rgba(180, 150, 210, 1)';
      // Draw multi-circle puff
      const puffs = [
        [0, 0, s],
        [-s * 0.6, s * 0.25, s * 0.72],
        [s * 0.65, s * 0.22, s * 0.75],
        [-s * 1.15, s * 0.45, s * 0.55],
        [s * 1.2, s * 0.42, s * 0.52],
        [-s * 0.2, s * 0.5, s * 0.68],
        [s * 0.35, s * 0.52, s * 0.62],
      ];
      ctx.beginPath();
    ...
    function animateSky(ts) {
      ...
      stars.forEach(star => {
        ...
        star.alpha = star.baseAlpha * (0.55 + 0.45 * Math.sin(star.phase));
        const x = star.x * w;
        const y = star.y * h;

        // Soft glow halo
        const grad = skyCtx.createRadialGradient(x, y, 0, x, y, star.r * 3.5);
        grad.addColorStop(0, `rgba(255, 240, 220, ${star.alpha})`);
        grad.addColorStop(1, 'rgba(255, 240, 220, 0)');
        skyCtx.fillStyle = grad;
        skyCtx.beginPath();
        skyCtx.arc(x, y, star.r * 3.5, 0, Math.PI * 2);
        skyCtx.fill();

        // Star core
        skyCtx.fillStyle = `rgba(255, 250, 240, ${Math.min(star.alpha * 1.6, 0.95)})`;
        skyCtx.beginPath();
        skyCtx.arc(x, y, star.r, 0, Math.PI * 2);
        skyCtx.fill();
      });
      ...
      clouds.forEach(cloud => {
        ...
        if (cloud.x * w > w + Math.min(w, h) * 0.3) cloud.x = -0.25;
        drawCloud(skyCtx, cloud, w, h);
      });
